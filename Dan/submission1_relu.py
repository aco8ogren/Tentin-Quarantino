...
def par_fun(args):
    ...
    for ind, state in enumerate(states_in_core):
        ...
        cube[0,:,ind] = state_to_fips[state]
        sys.stdout.flush()
...
def apply_by_mp(func, wokers, args):
    pool = multiprocessing.Pool(processes=workers)
    result = pool.map(funtest, args)
    pool.close()
    return result
...
if __name__ == '__main__':
    ...
    args = [5,6,7,8]
    workers = 4
    res = apply_by_mp(funtest,workers,args)

# %%

# num_states = len(state_to_fips)

# cube = np.zeros((100+1,num_days,num_states))
...
#     try:
#         if list_of_low_death_states.count(state) == 0:
#             daysofdata = state_to_daysofdata[state]
#             state_start_day = determine_state_start_day(state,global_dayzero,state_to_dayzero)
#             extrap = determine_extrapolate(daysofdata,state_start_day,num_days)
...
#             mobility_data = select_region(mobility_df, state,mobility = True)
#             tic = time.time()
#             res = least_squares(fit_leastsq_z, guesses, args=(data,mobility_data), bounds=np.transpose(np.array(ranges)),jac = '2-point')
#             all_s,D_quantiles,D,errors = plot_with_errors_sample_z(res, params, initial_conditions, df, mobility_df, state, extrapolate=extrap, boundary=boundary, plot_asymptomatic_infectious=False,plot_symptomatic_infectious=True)
#             all_s,D_quantiles,D,errors = plot_with_errors_sample_z(res, params, initial_conditions, df, mobility_df, state, extrapolate=extrap, boundary=boundary, plot_asymptomatic_infectious=False,plot_symptomatic_infectious=False)
#             toc = time.time()
#             print(toc-tic,' sec elapsed')

#             cube[1:,state_start_day:,counter] = all_s[:,:,5]
#     except:
#         print('Exception went off')
        
#     cube[0,:,counter] = state_to_fips[state]
#     counter = counter + 1
