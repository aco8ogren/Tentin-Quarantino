...
# %%
def plot_with_errors_sample_z(res, p0_params, p0_initial_conditions, df, mobility_df, region, d_thres, extrapolate=1, boundary=None, plot_asymptomatic_infectious=False,plot_symptomatic_infectious=True,):
...
def par_fun(states_in_core, main_df, mobility_df, coreInd, const, ErrFlag):
    ...
    for ind, state in enumerate(states_in_core):
        try:
            ...
            data = select_region(main_df, state, min_deaths=const['train_Dfrom'])#[:boundary]
            ...
            if boundary == 0:
                # Skip the state when train_til causes the state to not have enough data
                    # This occurs because the state had more than D_THRES deaths in the most recent dataset 
                    # BUT train_til occurs before the train_Dfrom threshold is met
                print('############################')
                print('Skipping a state:')
                print('core index: ', coreInd)
                print('   state with issue: %s'%state)
                print('train_til value leaves no trainable data for this state')
                print('############################')
                continue
            ...
            all_s, _, _, _ = plot_with_errors_sample_z(res, const['params'], const['initial_conditions'], main_df, mobility_df, state, const['D_THRES'], extrapolate=extrap, boundary=boundary, plot_asymptomatic_infectious=False,plot_symptomatic_infectious=False)
            toc = time.time()
...
if __name__ == '__main__':
    ...
    #-- Define list_of_states using the df directly instead of the dict(); this prevents issues with missing states in the df data
    #list_of_states = list(state_to_ticker.keys())
    list_of_states = list(np.unique(df['state']))
    ...
    for state in list_of_states:
        if state_to_maxdeaths[state]<=D_THRES:
    ...
    for state in list_of_states:
        if list_of_low_death_states.count(state)==0:
            print(state)
            d = select_region(df,state,min_deaths = D_THRES)
        # else:
        #     d = select_region(df,state,min_deaths = 1)
