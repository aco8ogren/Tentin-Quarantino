...
params = [2, 0.235, 0.2, 0.05, 0.32, 0.005]
# conditions: E, I, Q, R, SA
initial_conditions = [4.8e-1, 1e-3, 8.83e-5, 1.3e-4, 1e-3]

extrapolate = 2
s = model_z(params + initial_conditions, d, len(d)*extrapolate)
t = np.arange(0,len(d))
tp = np.arange(0,len(d)*extrapolate)

plt.scatter(t, d['cases'], color = 'red')
plt.plot(tp, s[:,3], color = 'red')
plt.scatter(t, d['deaths'],color ='black')
plt.plot(tp, s[:, 5],color ='black')
plt.show()
