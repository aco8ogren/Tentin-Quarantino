...
def sample_bootstrap_err(t, fit_func, fit_bounds, popt, errors, num_samples=100):
    ...
    all_deciles = np.transpose(np.array([np.percentile(all_samples, per, axis=0) for per in np.arange(.5, 100, 1)]))
    # Remove any spurious negative values
    all_deciles[all_deciles < 0] = 0
    return all_deciles, all_samples


def make_erf_quant_predictions(df, county_fips, key='deaths', last_date_pred='2020-06-30', start_date='2020-01-21',
                               boundary_date=None):
    '''
...
def predict_counties(df, list_of_fips, const, last_date_pred='2020-06-30', out_file='erf_model_predictions.csv', boundary_date=None,
    ...
    _, all_row_starts = utils.all_output_fips('sample_submission.csv')
    #num_dates = len(out_dates)
    num_fips = len(list_of_fips)
    num_dates = 161
    cube = np.zeros((101, num_dates, num_fips))
    ...
    for fips_idx, fips in enumerate(list_of_fips):
        ...
        all_deciles,integer_boundary_date = make_erf_quant_predictions(df, fips, last_date_pred=last_date_pred,
        ...
        if const['isSaveMatplot']:
            ...
            plt.scatter(actual_dates,actual_deaths,color = 'red')
            ylims = plt.gca().get_ylim()
            plt.vlines(integer_boundary_date,ylims[0],ylims[1])
            ptit = 'erf ' + str(fips)
            plt.title(ptit)
            folder_name = 'OutputPlots/' + const['save_time'] + ptit + '.pdf'
