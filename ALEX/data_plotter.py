...
#    p.circle(t_true, cnty_true_df['deaths'], color ='black')
    plt.figure()
    p1 = plt.plot(t_true, cnty_true_df['deaths'], 'ko', markersize = 3)
    plt.title(ptit)
    plt.xlabel('t (days since %s)'%global_dayzero.date())
...
#    bokeh.io.show(p)
    ...
    # for i,rel_date in enumerate(cnty_true_df['rel_date'].values):
    #     if i != 1:
    #         cnty_true_df[cnty_true_df['rel_date']==rel_date]['deaths_per_day'] = cnty_true_df[cnty_true_df['rel_date'] == (rel_date - 1)]['deaths'] - cnty_true_df[cnty_true_df['rel_date'] == (rel_date)]['deaths']
    total_deaths = cnty_true_df['deaths'].values
    deaths_per_day = np.diff(total_deaths,prepend = 0)
    cnty_true_df['deaths_per_day'] = list(deaths_per_day)
...
#                    title = ptit,
#                    x_axis_label = 't (days since %s)'%global_dayzero.date(),
#                    y_axis_label = '# deaths per day')
#
#    p.circle(t_true,cnty_true_df['deaths_per_day'], color = 'red')
    
    
    plt.figure()
    p2 = plt.plot(t_true, cnty_true_df['deaths_per_day'], 'ro', markersize = 3)
#    plt.title(ptit)
#    plt.xlabel('t (days since %s)'%global_dayzero.date())
#    plt.ylabel('# deaths per day')
               
    p2 = plt.plot(t_true, gaussian_filter(cnty_true_df['deaths_per_day'].values,3), 'b-')
    plt.title(ptit)
    plt.xlabel('t (days since %s)'%global_dayzero.date())
    plt.ylabel('# deaths per day')
